			...
			try{
				...
				if ($N_SOLDE<=0){
					...
					if ($NBJ==0) {	
						...
						$requete2->execute();
						$requete3=$connexion->prepare("UPDATE UTILISATEUR SET NB_J=$NN_SOLDE where MATRICULE=$mat");
						$requete3->execute();
							if ($requete0[0][1]==Chef) {
					...
					$requete5=$connexion->prepare("UPDATE UTILISATEUR SET NB_J=$NV_NBJ where MATRICULE=$mat");
					$requete5->execute();
					if ($requete0[0][1]==Chef) {
						header("location:validationadmin.php");
					}else{
