...
session_start();
			...
			try{
				...
				$requete0=$connexion->prepare("SELECT NB_J , ROLE from UTILISATEUR where MATRICULE=$mat");
				$requete0->execute();
				...
				if ($N_SOLDE<=0){
					...
					if ($NBJ==0) {
						...
						if ($NN_SOLDE <0) {
							...
							$requete->execute();
							$requete2=$connexion->prepare("UPDATE UTILISATEUR SET NB_J=$NN_SOLDE where MATRICULE=$mat");
							$requete2->execute();
							if ($requete0[0][1]==Chef) {
					...
					$requete2=$connexion->prepare("UPDATE UTILISATEUR SET NB_J=$NV_NBJ where MATRICULE=$mat");
					$requete2->execute();
					if ($requete0[0][1]==Chef) {
						header("location:mesdemandeschef.php");
					}else{
